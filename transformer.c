...
void transformer(logic_table* logic_table){
    for(int l = 0; l < logic_table->height; l++){
        ...
        for(int c = logic_table->input_width-1; c >= 0; c--){   // Table was parsed backwards to ensure expressions evaluate to logical 1
            char input_value = logic_table->header[c];  // Get the header (input value) for the current column
            char expression[8] = {};
            if(logic_table->body[l][c] == '0' && c > 0) snprintf(expression, sizeof(expression), "(%c%c)&", '~', input_value); // Value == '0' => '~' (NOT operator)
            if(logic_table->body[l][c] == '0' && c == 0) snprintf(expression, sizeof(expression), "(%c%c)", '~', input_value); 
            if(logic_table->body[l][c] == '1' && c > 0) snprintf(expression, sizeof(expression), "(%c)&", input_value); // Not end of the line => '&' (AND operator)
        ...
        if(l == logic_table->height - 1) strncat(logic_table->output_expression, top_expression, sizeof(top_expression));  // Produce the output expression
    }
}